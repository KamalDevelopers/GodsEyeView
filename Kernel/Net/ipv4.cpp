...
uint16_t IPV4::calculate_checksum(ipv4_packet_t* packet, uint32_t size)
{
...
bool IPV4::handle_packet(ipv4_packet_t* ipv4, uint32_t size)
{
    ...
    if (send) {
        uint32_t destination_ip = ipv4->destination_ip;
        ipv4->destination_ip = ipv4->source_ip;
        ipv4->source_ip = destination_ip;

        ipv4->ttl = 0x40;
        ipv4->checksum = 0;
        ipv4->checksum = calculate_checksum(ipv4, 4 * ipv4->header_length);
    }

    return send;
}

void IPV4::send_packet(uint32_t destination_ip, uint8_t* buffer, uint32_t size)
{
