...
#else
#    define CODE_BITS(n) ((n) < 4096 ? ((n) < 1024 ? 8 + ((n) >= 512) : 10 + ((n) >= 2048)) : ((n) < 16384 ? 12 + ((n) >= 8192) : 14 + ((n) >= 32768)))
#endif
...
static int w_buffer_ptr = 0;
static int w_buffer_index = 0;
static int w_size = 0;
void lzw_set_write_buffer(unsigned char* buffer, int size)
{
    w_size = size;
    w_buffer_index = 0;
    w_buffer_ptr = (uint32_t)buffer;
}
...
static void write_buff(int value)
{
    ...
    if (w_buffer_index == w_size)
        return;

    ((unsigned char*)w_buffer_ptr)[w_buffer_index] = value;
    w_buffer_index++;
    return;
}

static int r_buffer_ptr = 0;
static int r_buffer_index = 0;
static int r_size = 0;
void lzw_set_read_buffer(unsigned char* buffer, int size)
{
    r_size = size;
    r_buffer_index = 0;
    r_buffer_ptr = (uint32_t)buffer;
}

static int read_buff()
{
    if (r_buffer_index == r_size)
        return EOF;

    int value = ((unsigned char*)r_buffer_ptr)[r_buffer_index];
    r_buffer_index++;
    return value;
}
...
int lzw_compress(int maxbits)
{
    ...
    write_buff(maxbits - 9); // first byte in output stream indicates the maximum symbol bits
...
int lzw_decompress()
{
    ...
    if ((read_byte = (read_buff())) == EOF || (read_byte & 0xf8)) // sanitize first byte
        return 1;
    ...
    while (1) {
        ...
        if (code == maxcode) // sending the maximum code is reserved for the end of the file
            ...
            write_buff(code);              // literally and becomes our initial prefix
            next_string++;
            maxcode++;
        }
